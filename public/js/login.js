...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__user__ = __webpack_require__("./resources/assets/js/user.js");
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__tool__ = __webpack_require__("./resources/assets/js/tool.js");
...
var $telephoneInput = $('#bindTelephoneInput');
var $codeInput = $('#bindTelephoneCodeInout');
var $sendCodeBtn = $('#send-code-btn');
var $bindTelephoneBtn = $('#bind-submit-btn');
...
function checkErrors() {
    var error = __WEBPACK_IMPORTED_MODULE_3__tool__["default"].getParameterByName('error');
    if (error) {
        if (error === 'cancel') {
...
function checkPlatformLoginCallback() {
    var token = __WEBPACK_IMPORTED_MODULE_3__tool__["default"].getParameterByName('access_token');
    if (token) {
        __WEBPACK_IMPORTED_MODULE_1__config__["a" /* default */].setAccessToken(token);
        var user = new __WEBPACK_IMPORTED_MODULE_2__user__["a" /* default */]();
        user.fecth(function (userJson) {
            user.storeToLocal(userJson);
            __WEBPACK_IMPORTED_MODULE_0__bootstrap___default()('/');
        });
    }
}

function checkBindTelephone() {
    var token = __WEBPACK_IMPORTED_MODULE_3__tool__["default"].getParameterByName('bind_token');
    if (token) {
        $('#login-page').hide();
...
function checkLogin() {
    var token = __WEBPACK_IMPORTED_MODULE_1__config__["a" /* default */].getAccessToken();
    if (token) {
        __WEBPACK_IMPORTED_MODULE_0__bootstrap___default()('/');
    }
}
...
function initLoginForm() {
    $('#login-form').submit(function () {
        ...
        $.ajax({
            type: 'post',
            url: __WEBPACK_IMPORTED_MODULE_1__config__["a" /* default */].apiUrl + '/oauth/token',
            headers: __WEBPACK_IMPORTED_MODULE_1__config__["a" /* default */].getHeaders(),
            data: {
                username: username,
                password: password,
                grant_type: 'password',
                client_id: __WEBPACK_IMPORTED_MODULE_1__config__["a" /* default */].clientId,
                client_secret: __WEBPACK_IMPORTED_MODULE_1__config__["a" /* default */].clientSecret,
                scope: '*'
            },
            statusCode: {
                401: function _() {
                    __WEBPACK_IMPORTED_MODULE_1__config__["a" /* default */].deleteAccessToken();
                    toastr.error('用户名或者密码错误');
                },
                400: function _(response) {
                    var json = response.responseJSON;
                    var message = json.hint;
...
function getDevice() {
    ...
    if (!deviceId) {
        deviceId = __WEBPACK_IMPORTED_MODULE_3__tool__["default"].generateId(24);
...
function getRequestToken(callback) {
    $.ajax({
        type: 'get',
        url: __WEBPACK_IMPORTED_MODULE_1__config__["a" /* default */].apiUrl + '/services/request_token',
        data: {
            device: getDevice()
        },
        statusCode: {
...
function sendCodeSms(token, telephone) {
    $.ajax({
        type: 'get',
        url: __WEBPACK_IMPORTED_MODULE_1__config__["a" /* default */].apiUrl + '/services/send_sms_code',
        data: {
            telephone: telephone,
            device: getDevice(),
            token: token
...
function initBindTelephoneBtn() {
    $bindTelephoneBtn.click(function () {
        var telephone = $telephoneInput.val();
        var bindToken = __WEBPACK_IMPORTED_MODULE_3__tool__["default"].getParameterByName('bind_token');
...
function requestBindTelephone(telephone, device, token, bindToken, code) {
    $.ajax({
        type: 'post',
        url: __WEBPACK_IMPORTED_MODULE_1__config__["a" /* default */].apiUrl + '/wechat/merge',
        data: {
            telephone: telephone,
            device: device,
            token: token,
        ...
        __WEBPACK_IMPORTED_MODULE_1__config__["a" /* default */].setAccessToken(accessToken);
...
var User = function () {
    ...
    _createClass(User, [{
        key: 'fecth',
        value: function fecth(callback) {
            $.ajax({
                url: __WEBPACK_IMPORTED_MODULE_0__config__["a" /* default */].apiUrl + '/users/my',
        ...
        key: 'storeToLocal',
        value: function storeToLocal(json) {
            Cookies.set('user', json);
        }
    }, {
        key: 'storeCompamyTypeToLocal',
        value: function storeCompamyTypeToLocal(type) {
            Cookies.set('companyType', type);
        }
    }, {
        key: 'getFromLocal',
        value: function getFromLocal() {
            var json = Cookies.get('user');
            return json;
        }
    }]);
