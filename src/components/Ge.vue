<template>
    <div>
        <div class="modal fade" id="ge" aria-labelledby="ge" role="dialog"
  ...
  methods: {
    ...
    trendApproverChecker(params) {
      if (this.formData.condition.includes(params.key)) {
        const tempData = this.formData.condition.indexOf(params.key);
    ...
    dataInit() {
      const {
        name, description, approval_form_controls, form_id,
      } = this.importData;
