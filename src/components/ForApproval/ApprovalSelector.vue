<template>
    <div class="approval-text-container col-md-12 pl-0" v-show="options.length > 0">
        <span class="col-md-2 text-right pl-0" :class="data[0].required===1?'require':''" v-if="title || data[0].control_title">{{title || data[0].control_title}}</span>
        <select class="good-picker selectpicker col-md-10" :disabled="data[0].disabled?true:false" data-plugin="" :value="value" :data-live-search="isSelectable"
            ...
            <option v-if="data[0].disabled?true:false" :value="valueListener.name" selected></option>
...
export default {
     // 凡是多选，都有搜索框；不是多选传入selectable为true也可以有搜索框
        ...
        props: ['n', 'multiple', 'placeholder', 'changeKey' , 'value', 'resetinfo', 'selectable','title','data','index','clear','directionalSender'],
        data() {
            return {
