                ...
                <div data-role="content" class="scrollable-content" style="width: 259px;">
                    <section class="page-aside-section">
                        ...
                        <div class="list-group">
                            <div class="list-group-item" :class="item.value == isShow?'selected':''"
                                 v-for="(item,index) in leftData.topData" :key="index"
                                 @click="switchMenu(item.value)">
                                <router-link :to="item.url">{{item.name}}<span
                                        v-if="item.name =='我的消息'&&unReadMsg>0"
                                        class="unRead ml-5">{{unReadMsg}}</span></router-link>
                            </div>
                        </div>
                    </section>
                    <section class="page-aside-section">
                        <div class="site-menubar-body" style="width:260px;">
                            <ul class="menu pl-0">
                                <li class="site-menu-item has-sub" v-for="(item,index) in leftData.bottomData"
                                    :key="index" @click="switchMenu(item.value)">
                                    <a class="pl-30 py-10" href="javascript:void(0)"
                                       :class="item.value == isShow?'selected':''">
                                    ...
                                    <ul class="site-menu-sub pl-45" v-show="isShow == item.value">
                                        <li class="site-menu-item" v-for="(item2,index) in  item.data" :key="index">
                                            ...
                                            <router-link :to="{path:item2.url,query:{id:item2.params.id}}"
                                                         v-if="item2.params">
                ...
                <div data-role="container" class="scrollable-container" style="height: 691px; width: 259px;">
                    <div data-role="content" class="scrollable-content" style="width: 259px;">
                        <section class="page-aside-section">
                            <h5 class="page-title pl-30 mb-45">{{title}}</h5>
                            <div class="list-group">
                                <router-link class="list-group-item" v-for="(item,index) in leftData.topData"
                                             :key="index"
                                ...
                                <ul class="menu pl-30">
                                    <li class="site-menu-item has-sub" v-for="(item,index) in leftData.bottomData"
                                        ...
                                        <ul class="site-menu-sub" v-show="isShow == item.value">
                                            <li class="site-menu-item" v-for="(item2,index) in  item.data"
                                                :key="index">
                                                <router-link :to="{path:item2.url,query:{id:item2.params.id}}"
                                                             v-if="item2.params">
