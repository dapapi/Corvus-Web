...
export default {
  ...
  computed: {
    ...
    typeHandler() {
      ...
      } if (this.currentStatus === 'economic') {
  ...
  methods: {
    ...
    getList(params) {
      ...
      const _this = this;
      fetch('get', `/approvals_contract/${this.currentStatus}`).then((response) => {
        _this.pageList = response.data;
    ...
    changeCommunicationType(value) {
      this.communication = value;
    },

    changeSignIntention(value) {
    ...
    changeWeiboFansNum(value) {
      this.star_weibo_infos.avatar = value;
    },

    changeDouyinFansNum(value) {
      this.star_douyin_infos.avatar = value;
    },
    changeArtistType(value) {
    ...
    selectArtists(value) {
      if (value === 'all') {
        ...
        const index = this.selectedArtistsArr.indexOf(value);
        if (index > -1) {
